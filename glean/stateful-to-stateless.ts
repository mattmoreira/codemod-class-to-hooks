...
export function statefulToStateless(component: string, isCallArgument: boolean) {
  const functionBody = [];
      ...
      if (hooksSupported()) {
        functionBody.push(
          namedArrowFunction({
            name,
            body,
            arrowFunctionExpression: buildUseCallbackHook({
              CALLBACK: arrowFunction([], [], body, isAsync),
            }).expression,
          })
        );
      } else {
    ...
    ClassDeclaration(path) {
      ...
      const propType =
        path.node.superTypeParameters &&
          ...
          ? path.node.superTypeParameters.params : null;

      const params = ["props"]
      const paramDefaults = defaultPropsPath ? [defaultPropsPath.node.value] : []
      ...
      if (isCallArgument) {
        const anonymousStatelessComponent = arrowFunction(
          params,
          paramDefaults,
          functionBody,
          false
        ...
        mainPath.insertBefore(anonymousStatelessComponent);
